...
const Multiples = (props) => {
	...
	//
	// )
	let multiplesArr = [<img src={ props.card.info.image_uris ? props.card.info.image_uris.small : null } style={{ position: 'absolute' }} className={styles.cardImage} alt="magic card" height="100px" />];
	for(let i = 2; i <= props.card.number; i++){
		let margin=5*(i-1);

		multiplesArr.push(<img src={ props.card.info.image_uris ? props.card.info.image_uris.small : null } style={{ position: 'absolute', zIndex: i, marginTop: `${margin}px`, marginLeft: `${margin}px`}} className={styles.cardImage} alt="magic card" height="100px" />)
	}
		...
		<div className={styles.deckWrapper}>
			...
			<div className={styles.deckGrid}>
				{props.deck.map((card, index) => {
					if(card.number>1) {
						return <Multiples key={index} card={card}/>
					} else {
						return (
